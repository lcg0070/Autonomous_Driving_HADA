...
char DBSCAN_processing(USTRectData rec_data, int* labels, int num_label, double* out_array){
    ...
    for(int i = 0; i < num_label; i++){
        ...
        for(int j = 0; j < each_count[i]; j++){
            ...
            obstacle_degree = RAD2DEG_2D*atan2(cluster_xy[3 * i + 1], cluster_xy[3 * i]);
...
//    cv::imshow("cluster approx", obstacle_data);
    ...
    // u_turn flag
    char flag;
    flag = cal_u_turn_flag(cluster_xy, num_label);
    flag = cal_min_radius_turn_flag(cluster_xy, num_label);
...
void circle_approximation(double* obstacle_xy,int obstacle_num, double* out_array)
{
    ...
    for(int i = 0; i < obstacle_num ; i++)
    {
        ...
        {
            double degree = DEG2RAD_2D*(OBSERVE_DEGREE - RESOLUTION * (float)j);
...
void visual_cluster(USTRectData rec_data, int* labels, double* out_array, float packet_receive_data, float packet_send_data, int flag){
    ...
    for(int i=0; i<INTERESTED_INDEX; i++){
        double radian = ((double)OBSERVE_DEGREE - RESOLUTION * ((double)i + INTERESTED_OFFSET)) * DEG2RAD_2D;
    ...
    // cvt gray 2 3ch
    cv::cvtColor(original_visual_data, original_visual_data, cv::COLOR_GRAY2BGR);
    cv::cvtColor(final_data, final_data, cv::COLOR_GRAY2BGR);

    std::vector<cv::Mat> images = {original_visual_data, DBSCAN_visual_data, final_data};
    cv::Mat combined_image;
    cv::vconcat(images, combined_image);


    std::stringstream ss;
    ss << "Steer ang = " << std::fixed << std::setprecision(2) << packet_receive_data << "  ";
    ss << "Steer to Control = " << std::fixed << std::setprecision(2) << packet_send_data << "  ";
    ss << "flag = " << flag;

    cv::putText(combined_image, ss.str(), cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 0), 2);

    cv::imshow("combined_img", combined_image);
...
double steer_command(double* off, double sff_degree){
    ...
    for(int i = 0; i < INTERESTED_INDEX; i++)
    {
        ...
        sff   = RS * exp(-SQR(angle - sff_degree) / (2.0 * SQR(SIGMA)));
...
char cal_u_turn_flag(double* cluster_xy, int num_label){
    for(int i = 0; i < num_label; i++) {
        double x = cluster_xy[3 * i];
        double y = cluster_xy[3 * i + 1];
        if (x == 0. || y == 0.) continue;
        if (y > -UTURN_THERESHOLD_Y && y < UTURN_THERESHOLD_Y) {
            // printf("x = %3.2f\ty = %3.2f\n", x,y);
            if (x < UTURN_THERESHOLD_X) return U_TURN_FLAG;
        }
    }
    // printf("==========================================\n");
    return 0;
...
char cal_min_radius_turn_flag(double* cluster_xy, int num_label){
    for(int i = 0; i < num_label; i++) {
        double x = cluster_xy[3 * i];
        double y = cluster_xy[3 * i + 1];
        double r = cluster_xy[3 * i + 2];

        if (x == 0. || y == 0.) continue;       // wrong obstacle
        double obstacle_distance = hypot(x,y);

        if (obstacle_distance < r) return -1;   // crash to obstacle

        double factor = 1 - r / obstacle_distance;
        double closest_x = x * factor;
        double closest_y = y * factor;

        double r1 = hypot(closest_x - MIN_TURN_RADIUS, closest_y);
        double r2 = hypot(closest_x + MIN_TURN_RADIUS, closest_y);
            if (r1 < MIN_TURN_RADIUS + CAR_HALF_WIDTH && r2 < MIN_TURN_RADIUS + CAR_HALF_WIDTH) {
                return MIN_RADIUS_FLAG;
            }
    }
    return 0;
}
